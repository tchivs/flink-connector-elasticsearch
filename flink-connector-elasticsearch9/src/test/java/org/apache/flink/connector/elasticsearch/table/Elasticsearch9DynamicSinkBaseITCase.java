...
/**
 * {@link Elasticsearch9DynamicSinkBaseITCase} is the base class for integration tests.
 *
...
abstract class Elasticsearch9DynamicSinkBaseITCase {
    ...
    TestContext getPrefilledTestContext(String index) {
        TestContext testContext =
                context()
                        .withOption(Elasticsearch9ConnectorOptions.INDEX_OPTION.key(), index)
                        .withOption(
                                Elasticsearch9ConnectorOptions.HOSTS_OPTION.key(),
                                secure
        ...
        if (secure) {
            testContext
                    .withOption(
                            Elasticsearch9ConnectorOptions.USERNAME_OPTION.key(),
                            ES_CLUSTER_USERNAME)
                    .withOption(
                            Elasticsearch9ConnectorOptions.PASSWORD_OPTION.key(),
                            ES_CLUSTER_PASSWORD)
                    .withOption(
                            Elasticsearch9ConnectorOptions.SSL_CERTIFICATE_FINGERPRINT.key(),
    ...
    String getConnectorSql(String index) {
        if (secure) {
            return String.format("'%s'='%s',\n", "connector", "elasticsearch-9")
                    + String.format(
                            "'%s'='%s',\n",
                            Elasticsearch9ConnectorOptions.INDEX_OPTION.key(), index)
                    + String.format(
                            "'%s'='%s',\n",
                            Elasticsearch9ConnectorOptions.HOSTS_OPTION.key(),
                            "https://" + ES_CONTAINER_SECURE.getHttpHostAddress())
